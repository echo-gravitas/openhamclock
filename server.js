...
const ROTATOR_PROVIDER = (
  process.env.ROTATOR_PROVIDER || 'pstrotator_udp'
).toLowerCase();
...
function ensureRotatorSocket() {
  ...
  sock.on('message', (buf, rinfo) => {
    ...
    console.log(`[Rotator] RX from ${rinfo.address}:${rinfo.port} -> "${s}"`);
...
async function queryAzimuthOnce(timeoutMs = 800) {
  ...
  rotatorInflight = rotatorInflight.then(async () => {
    ...
    try {
      ...
      // <PST>AZ?</PST>
      console.log(
        `[Rotator] TX query -> ${PSTROTATOR_HOST}:${PSTROTATOR_UDP_PORT}`,
      );
...
app.get('/api/rotator/status', async (req, res) => {
  ...
  const live2 =
    rotatorState.azimuth !== null &&
    now2 - rotatorState.lastSeen <= ROTATOR_STALE_MS;
