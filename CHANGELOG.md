...
- **International weather → client-direct** — Moved Open-Meteo fetching from server to each user's browser. With 2,000+ users, our single server IP was exhausting Open-Meteo's free tier (10K/day, 600/min), causing constant 429 backoff spirals. Now each user's browser calls `api.open-meteo.com` directly — rate limits are distributed across all user IPs instead of concentrated on our server. Server handles US weather only via NWS (unlimited, no key needed). Optional Open-Meteo API key field added to Settings for users who want higher limits. Removed ~200 lines of server-side Open-Meteo code (queue, batch fetch, backoff management)
